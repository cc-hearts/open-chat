...
| components               | 将 HTML 节点映射为自定义 Vue 组件                                                            | `Record<string, Component \| keyof HTMLElementTagNameMap>`                          | -               |
...
| paragraphTag             | 段落使用的 HTML 标签（避免自定义组件含块级元素时的校验问题）                                 | `keyof HTMLElementTagNameMap`                                                       | `'p'`           |
